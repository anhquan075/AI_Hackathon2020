...
def full_flow(INP_DIR, fold=1):
    ...
    # label = predict_efficientNetB8(CROP_DIR, BATCH_SIZE=8, MODEL_PATH='model/checkpoint-4.pth.tar')

    SUB_DIR_PATH = os.path.dirname(SUB_PATH)
    SUB_FILE_PATH = os.path.basename(SUB_PATH)

    OUT_SUB_PATH, OUT_SUB_PATH_VIZ = post_proc(SUB_DIR_PATH, SUB_FILE_PATH, 'submission', 'sub_test_flow_{}.txt'.format(str(1)), INP_DIR)
